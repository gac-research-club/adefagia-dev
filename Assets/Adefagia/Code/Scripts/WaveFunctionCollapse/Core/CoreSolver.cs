...
namespace Adefagia.WaveFunctionCollapse
{
    public class CoreSolver
    {
        ...
        public void Propagate()
        {
            ...
            if (
                propagationHelper.CheckForConflicts()
        ...
        {
            var possibleValuesAtNeighbour = outputGrid.GetPossibleValueForPossition(
            ...
            propagationHelper.AnalyzePropagationResults(
                propagatePair,
                startCount,
                newPossiblePatternCount
            );
        }

        private void RemoveImpossibleNeighbours(
            VectorPair propagatePair,
            HashSet<int> possibleValuesAtNeighbour
        )
        {
            HashSet<int> possibleIndices = new HashSet<int>();

            foreach (
                var patternIndexAtBase in outputGrid.GetPossibleValueForPossition(
        ...
        public void CollapseCell(Vector2Int cellCoordinates)
        {
            ...
            else
            {
                ...
                outputGrid.SetPatternOnPosition(
                    cellCoordinates.x,
                    cellCoordinates.y,
                    possibleValue[index]
                );
