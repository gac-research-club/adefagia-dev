...
namespace Adefagia.WaveFunctionCollapse
{
    public class NeighboursStrategySize2andMore : IFindNeighbourStrategy
    {
        ...
        )
        {
            ...
            {
                foreach (
                    var possibleNeighbourForPattern in patternFinderResult.PatternIndexDictionary
                )
                {
                    FindNeighboursInAllDirections(
                        result,
                        patternDataToCheck,
                        possibleNeighbourForPattern
                    );
                }
            }
            return result;
        ...
        private void FindNeighboursInAllDirections(
        ...
        )
        {
            foreach (Direction dir in Enum.GetValues(typeof(Direction)))
            {
                if (patternDataToCheck.Value.CompareGrid(dir, possibleNeighbourForPattern.Value))
                {
                    ...
                    result[patternDataToCheck.Key].AddPatternToDictionary(
                        dir,
                        possibleNeighbourForPattern.Key
                    );
                }
            }
        }
