...
namespace Adefagia.PlayerAction
{
    public class HighlightMovement : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject quadMove, quadAttack, quadSupport, quadDeploy;
        ...
        public void SetAreaTeamDeploy(TeamController teamActive, GridManager gridManager)
        {
            if (!teamActive.HasAreaHighlight) return;
            int _row = gridManager.GridSizeX;
            int _column = gridManager.GridSizeY;
            ...
            for (int row = 0; row < _row; row++)
            {
                ...
                {
                    Grid gridDeploy = gridManager.GetGrid(row, column);
                    if (gridDeploy == null) continue;
                    if (teamActive.IsGridInPreparationArea(gridDeploy) && !Grid.IsOccupied(gridDeploy) && gridDeploy.Status != GridStatus.Obstacle)
                    {
                        AddGridHighlight(gridDeploy);
                    }
                }
            }

            CreateHighlightObject(_tempGrids);
        ...
        private void CreateHighlightObject(List<Grid> highlightGrids)
        {
            var removedGrid = new List<Grid>();
            foreach (var highlightGrid in highlightGrids)
            {
        ...
        private void SetQuad()
        {
            if (BattleManager.battleState == BattleState.MoveRobot)
            {
                ...
                _quadBlock = quadMoveBlock;
                     ...
                     BattleManager.battleState == BattleState.SkillSelectionRobot)
            {
                Skill skill = BattleManager.TeamActive.RobotControllerSelected.Robot.SkillSelected;
